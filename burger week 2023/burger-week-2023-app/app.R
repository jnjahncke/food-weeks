...
burger_week <- burger_week %>% mutate(interest_level = 5,
...
ui <- fluidPage(
    ...
    # Application title
    titlePanel("Burger Week 2023"),
    
    # Sidebar
    sidebarLayout(
        sidebarPanel(
            actionButton("go", label = "Update Map"),
            sliderInput(inputId = "days", label = "How many days long is your burger crawl?", min = 1, max = 7, value = 1),
            sliderInput(inputId = "vote",label = "Only show burgers I am interested in. Interest threshold:", min = 1, max = 5, value = 3),
            checkboxGroupInput(inputId = "meat_veg", label = "Meat or Vegetarian?", choices = c("Meat", "Vegetarian", "Vegan"), selected = c("Meat", "Vegetarian", "Vegan")),
            ...
            checkboxGroupInput(inputId = "disp_cols", label = "Choose which columns to display:", choices = c("restaurant","burger","toppings","address","interest_level","cluster","hours","meat_veggie","gluten_free","vegan","veggie","minors","takeout","delivery","purchase_limit"),
                               selected = c("restaurant","burger","toppings","address","interest_level","cluster")),
...
server <- function(input, output) {
    ...
    output$download2 <- downloadHandler(filename = function() {"votes.csv"},
                                        content = function(fname) {write.csv(v$data %>% select(restaurant, burger, toppings, address, interest_level), fname)})
    
    # for casting votes
    v <- reactiveValues(data = burger_week %>% select(restaurant, burger, toppings, address, interest_level))
    ...
    # filter, cluster
    filter_db <- reactive({
        votes <- burger_week %>% select(-interest_level) %>% 
            filter(
                grepl(pattern = meat_veg_pattern(), x = meat_veggie),
                grepl(pattern = ifelse(input$takeout == "Takeout", "Yes", "."), x = takeout),
                grepl(pattern = ifelse(input$gluten == "Either", ".", "Yes|Maybe"), x = gluten_free),
    ...
    output$download1 <- downloadHandler(filename = function() {"burgers.csv"},
                                        content = function(fname) {write.csv(filter_db() %>% select(input$disp_cols), fname)})
    
    # display filtered results
    output$burger_db <- renderDT({
    ...
    output$download3 <- downloadHandler(
        ...
        content = function(fname) {
            write.csv(
                filter_db() %>% select(cluster, restaurant, burger, address, hours) %>% arrange(cluster),
                fname
            )
        }
    )
    ...
    output$schedule <- renderDataTable({
        ...
        # display table
        votes %>%
            select(cluster, restaurant, burger, address, hours) %>%
            arrange(cluster) %>%
            datatable(rownames = FALSE, options = list(lengthMenu = c(5, 10, 25, 50, 100), pageLength = 100))
    })
